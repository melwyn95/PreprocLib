...
let rec drop n i = function
  _::l when i < n -> drop n (i+1) l
| l -> l

let drop n = drop n 0

let drop_first_char_opt string =
  try Some (String.sub string 1 (String.length string - 1)) with
...
let compact (list: 'a option list) : 'a list option =
  let folded x acc =
    let* acc in
    let* x   in
    Some (x :: acc)
  in List.fold_right folded list (Some [])
...
module JsonHelpers =
  struct
    ...
    let string_list_of (json : Yojson.Basic.t) =
      match json with
        `List l -> compact @@ List.map string_of l
      | _ -> None
...
module Path =
  struct
    ...
    let segs : t -> string list option = Option.map Fpath.segs
    ...
    let dirpath path =
      path
    |> Option.map Fpath.split_base
    |> Option.map fst
    |> Option.map Fpath.rem_empty_seg
...
let clean_installation_json ~root abs_path_to_project_root installation_json =
  ...
  in List.fold_left folded (Some PackageMap.empty) kvs
...
let clean_lock_file_json lock_json =
  ...
  let  folded map (key, value) =
    ...
    let  dependencies = List.map (fun d -> Package d) dependencies
    in Some (PackageMap.add key dependencies map) in
  let* node = List.fold_left folded (Some PackageMap.empty) kvs
  in Some {root; node}
    ...
    then graph
      ...
      match deps with
        Some deps ->
          ...
          in List.fold_left dfs graph deps
      | _ -> graph
...
| Some {root_path; resolutions} ->
    let root_inclusion_paths =
      List.find_opt (fun (p,_) -> equal_paths p root_path) resolutions
...
| Some {resolutions; _} ->
    ...
    let rec aux path =
      ...
      let resolution = List.find_opt predicate resolutions
...
(* The call [find_external_file ~file ~inclusion_paths] specifically
   ...
   To correctly resolve the path for #include we split the
...
let find_external_file ~file ~inclusion_paths =
  let find_package_root_dir ?scope pkg_name =
    ...
    in List.find_opt predicate inclusion_paths
  in
...
(* The function [print] pretty-prints module resolutions. *)

let print ppf (mod_res : t) =
